...
pub const ACCOUNT_ADDRESS_SIZE: usize = 42;
...
impl convert::AsRef<[u8; ACCOUNT_ADDRESS_SIZE]> for AccountAddress {
    fn as_ref(&self) -> &[u8; ACCOUNT_ADDRESS_SIZE] { &self.0 }
}
...
impl From<String> for AccountAddress{
    fn from(address: String) -> Self{
        let adr = address.as_bytes();
        let mut aadr:[u8; ACCOUNT_ADDRESS_SIZE] = [0; ACCOUNT_ADDRESS_SIZE];
        aadr.clone_from_slice(&adr[0..ACCOUNT_ADDRESS_SIZE]);
        AccountAddress(aadr)
    }
}
...
impl From<AccountAddress> for String{
    fn from (address: AccountAddress) -> Self{
        std::str::from_utf8(&address.0).unwrap().to_string()
    }
}

#[cfg(not(feature = "derive-serde"))]
impl serde::Serialize for AccountAddress {
    fn serialize<S>(&self, serializer: S)->Result<S::Ok, S::Error>
        where
            S: serde::Serializer,
    {
        let mut seq = (serializer.serialize_tuple(ACCOUNT_ADDRESS_SIZE))?;
        for e in self.0{
...
pub type ContractAddress = AccountAddress;
...
pub struct ChainMetadata {
    ...
    pub height: u64, //xq api
    pub tx_hash: String, //xq tx hash
}
...
impl DID{
    pub fn validate_vc(self, vc:String)-> i32{
        // return  match validate_vc_by_string(vc)
        // {
        //     Ok(true) => 1i32,
        //     Ok(false) => 0i32,
        //     Err(_) => -1i32,
