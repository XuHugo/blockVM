...
pub struct Contract {
    ...
    pub event:   Option<Type>,   //xq log-event
...
impl Serial for Contract {
    fn serial<W: Write>(&self, out: &mut W) -> Result<(), W::Err> {
        ...
        self.event.serial(out)?;  //xq log event
        ...
        pub fn to_json<R: Read>(&self, source: &mut R) -> ParseResult<serde_json::Value> {
            ...
            match self {
                ...
                Type::AccountAddress => {
                    ...
                    //xuqiang  Standard ToString is not appropriate and requires a self-defined method
                    Ok(Value::String(String::from(address)))
