...
pub fn write_bytes_from_json_schema_type<W: Write>(
    ...
    match schema {
        ...
        Type::ContractAddress => {
            if let Value::String(string) = json {
                let account: ContractAddress =
                    string.parse().map_err(|_| anyhow!("Failed parsing contract address"))?;
                serial!(account, out)
            } else {
                bail!("JSON String required")
            }
        ...
        Type::Array(len, ty) => {
            if let Value::Array(values) = json {
                ...
                bail!("JSON Array required array")
            }
        }
