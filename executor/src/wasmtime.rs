...
pub fn init_wasm(func_name: &str, context: Context, binary: &[u8], amount: i64) -> Result<InitResult> {
    ...
    //let engine = Engine::new(Config::new().interruptable(true).consume_fuel(true))?;
    ...
    let run = match instance.get_func(&mut store, func_name){
        Some(r) => r,
        None => return Err(anyhow!("can't find init func from module!"))
    };

    let ret = match run.call(&mut store, &[I64(amount)]){
        Ok(r) => {
            //{println!("init_wasm ok!!!{:?}",r[0].i32())};
            Some(r[0].clone())
        },
        Err(trap) => {
            //{println!("init_wasm  error!!,{}", trap)};
            return Err(trap)
        },
    };
    println!("fib => wasmtime cost init:{:?} ms",start.elapsed().as_millis());
    // println!("store data integers_logged:{:?}!",store.data().integers_logged);
    // println!("store data origin:{:?}!",store.data().origin);
    // println!("store data state:{:?}!",store.data().state);
    // println!("store data param:{:?}!",store.data().param);
    // println!("store data logs:{:?}!",store.data().logs);
    // println!("store data actions:{:?}!",store.data().outcomes.cur_state);
    //println!("fuel-init:{}",store.fuel_consumed().unwrap());
...
pub fn receive_wasm(func_name: &str, context: Context, binary: &[u8], amount: i64) -> Result<ReceiveResult> {
    ...
    {
        Some(r) => r,
        None => return Err(anyhow!("can't find receive func from module!"))
    };

    let ret = match run.call(&mut store, &[I64(amount)]){
        Ok(r) => {
            //{println!("receive_wasm start ok!!!{:?}",r[0].i32())};
            Some(r[0].clone())
        },
        Err(trap) => {
            //println!("@@@@@@@@@@@@receive_wasm start  error!!,{}", trap);
            return Err(trap)
        },
    };
    ...
    // println!("store data param:{:?}!",store.data().param);
    // println!("store data logs:{:?}!",store.data().logs);
    // println!("store return return:{:?}!",store.data().returndata);
    // println!("fuel-recv:{}",store.fuel_consumed().unwrap());
    if let Some(Val::I32(n)) = ret {
        if store.data().gas_outof{
            println!("1111");
...
// call contract
pub fn receive_wasm_aot(func_name: &str, context: Context, binary: &[u8], amount: i64) -> Result<ReceiveResult> {
