...
use aptos_vm::data_cache::{StorageAdapterOwned, IntoMoveResolver};
...
impl GeecoDataStore {
    ...
    pub fn new(data: HashMap<StateKey, Vec<u8>>) -> Self {
        GeecoDataStore { state_data: data }
    }
    ...
    pub fn add_write_set(&mut self, write_set: &WriteSet) {
        for (state_key, write_op) in write_set {
            ...
            //         println!("access key:{:?}", access_path.address.to_hex_literal());
            //     }
            //     StateKey::TableItem { handle, key } => {
            //         println!("table key:{:?}",handle.0.to_hex_literal());
            //     }
            ...
            match write_op {
                WriteOp::Modification(blob) | WriteOp::Creation(blob) => {
                    //println!("key:{:?}",state_key.clone());
                    //println!("value:{:?}",blob.clone());
...
impl StateView for GeecoDataStore {
...
impl GeecoDataStore{
    ...
    // pub fn move_resolver(self) -> StorageAdapterOwned<GeecoDataStore> {
    //     self.into_move_resolver()
        
    // }

    // fn get(&self, key: &StateKey) -> Result<Option<Vec<u8>>> {
    //     Ok(self.state_data.get(key).cloned())
    // }
}



pub struct GeecoDbReader {}

// impl aptos_storage_interface::DbReader for GeecoDbReader {
