...
use aptos_types::{access_path::AccessPath, vm_status::StatusCode};
...
pub struct TxnLastInputOutput<K, T, E> {
    pub inputs: Vec<CachePadded<ArcSwapOption<TxnInput<K>>>>, // txn_idx -> input.

    pub outputs: Vec<CachePadded<ArcSwapOption<TxnOutput<T, E>>>>, // txn_idx -> output.
...
impl<K: ModulePath, T: TransactionOutput, E: Send + Clone> TxnLastInputOutput<K, T, E> {
    ...
    pub fn record(
        ...
        //print!("[Ereadset:{:?}]",input.len());
        self.inputs[txn_idx].store(Some(Arc::new(input)));
        self.outputs[txn_idx].store(Some(Arc::new(output)));
        //print!("[recordfunc({:?}):{:?}]",std::thread::current().name(),self.display_keys(txn_idx));
        // match output {
        ...
        //         print!("[record Success:{:?}]",output.get_writes().iter().map(|(k,_)| k).collect());
        //     }
        //     ExecutionStatus::SkipRest(output) => {
        //         // Apply the writes/deltas and record status indicating skip.
        //         print!("[record Skip:{:?}]",output.get_writes().iter().map(|(k,_)| k).collect());
        //     }
        //     ExecutionStatus::Abort(err) => {
        //         // Record the status indicating abort.
        //         print!("[record Abort:{:?}]",err.into());
        //     }
        // };
    }
    ...
    pub fn display_keys(&self, txn_idx: TxnIndex) -> KeySet<T> {
        match &self.outputs[txn_idx].load_full() {
            None => HashSet::new(),
            Some(txn_output) => match txn_output.as_ref() {
                ExecutionStatus::Success(t) | ExecutionStatus::SkipRest(t) => t
    ...
    pub fn get_status(&self, txn_idx: TxnIndex) -> Option<bool> {
        match &self.outputs[txn_idx].load_full() {
            None => None,
            Some(txn_output) => match txn_output.as_ref() {
                ExecutionStatus::Success(t) | ExecutionStatus::SkipRest(t) => Some(true),
                ExecutionStatus::Abort(_) => Some(false),
            },
        }
    }

    //xqadd
    pub fn get_error(&self, txn_idx: TxnIndex) -> Option<StatusCode> {
        match &self.outputs[txn_idx].load_full() {
            None => None,
            Some(txn_output) => match txn_output.as_ref() {
                ExecutionStatus::Success(t) | ExecutionStatus::SkipRest(t) => None,
                ExecutionStatus::Abort(e) => {
                    let code = match e{
                        Error::ModulePathReadWrite => StatusCode::UNKNOWN_VALIDATION_STATUS,
                        Error::UserError(_) => StatusCode::UNKNOWN_VALIDATION_STATUS,
                        Error::NONCE_TOO_NEW => StatusCode::SEQUENCE_NUMBER_TOO_NEW,
                    };
                    Some(code)
                },
            },
