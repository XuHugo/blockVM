...
impl Scheduler {
    ...
    pub fn next_task(&self) -> SchedulerTask {
        //xuq
        //print!("[Next_task({:?}):]",std::thread::current().name());
        loop {
            if self.done() {
                // No more tasks.
                //xuq
                //println!("[Done({:?}):]",std::thread::current().name());
            ...
            if idx_to_validate < idx_to_execute {
                if let Some((version_to_validate, guard)) = self.try_validate_next_version() {
                    //xuq
                    //println!("[Vtask({:?}):{:?} end:({}+{})]",std::thread::current().name(),version_to_validate,idx_to_execute,idx_to_validate);
            ...
            {
                //xuq
                //println!("[Etask({:?}):{:?}end:({}+{})]",std::thread::current().name(),version_to_execute,idx_to_execute,idx_to_validate);
    ...
    pub fn finish_execution<'a>(
        ...
        if self.validation_idx.load(Ordering::SeqCst) > txn_idx {
            if revalidate_suffix {
                ...
                //xuq
                //print!("[newVTask({:?})]",std::thread::current().name());
                return SchedulerTask::ValidationTask((txn_idx, incarnation), guard);
            }
        }
        //xuq
        //print!("[noTask({:?})]",std::thread::current().name());
    ...
    pub fn finish_abort<'a>(
        ...
        if self.execution_idx.load(Ordering::SeqCst) > txn_idx {
            ...
            if let Some((new_incarnation, maybe_condvar)) = self.try_incarnate(txn_idx) {
                //xuq
                //print!("[Etask({:?})({}-{})]",std::thread::current().name(),txn_idx, new_incarnation);
        ...
        //xuq
        //print!("[Notask1({:?})]",std::thread::current().name());
        SchedulerTask::NoTask
    }

    pub fn check_validate(&self, txn_idx: TxnIndex,)  -> bool{
        let idx_to_validate = self.validation_idx.load(Ordering::SeqCst);
        //xqtest
        // if txn_idx == idx_to_validate{
        //     println!("[Check==:{:?}/{:?}]",txn_idx, idx_to_validate);
        // }else{
        //     println!("[Check<>:{:?}/{:?}]",txn_idx, idx_to_validate);
        // }
        
        // if txn_idx > idx_to_validate + 2{
        //     return true
        // }
        false
    }
...
impl Scheduler {
    ...
    fn decrease_validation_idx(&self, target_idx: TxnIndex) {
        ...
        if n > target_idx {
            //xuq
            //print!("[deVid({:?})({},{})]",std::thread::current().name(),n,target_idx);
            self.decrease_cnt.fetch_add(1, Ordering::SeqCst);
            //println!("[deVid({:?})({},{}){:?}]",std::thread::current().name(),n,target_idx,self.decrease_cnt);
        }
    }
    ...
    fn decrease_execution_idx(&self, target_idx: TxnIndex) {
        ...
        if n > target_idx {
            print!("[deEid({:?})({},{})]",std::thread::current().name(),n,target_idx);
            self.decrease_cnt.fetch_add(1, Ordering::SeqCst);
        }
    }
