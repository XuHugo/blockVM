...
use crate::{
    ...
    task::{ExecutionStatus, ExecutorTask, ModulePath, Transaction, TransactionOutput, self},
...
use aptos_types::{write_set::TransactionWrite, vm_status::StatusCode};
use num_cpus;
use once_cell::sync::Lazy;
use std::{collections::{btree_map::BTreeMap, HashSet}, hash::Hash, marker::PhantomData, sync::Arc, fmt::Debug};
    ...
    E: ExecutorTask<T = T>,
{
    ...
    fn execute<'a>(
        ...
        //xuq
        //print!("[Execute({:?}){}-{}:]",std::thread::current().name(),idx_to_execute,incarnation);
        ...
        let mut apply_updates = |output: &<E as ExecutorTask>::Output| {
            ...
            for (k, v) in output.get_writes().into_iter() {
                if !prev_modified_keys.remove(&k) { //&& incarnation!=0
                    //print!("[@{},{}updates_outside:{:?}]",idx_to_execute, incarnation,k);
                   // print!("[@{},{}outside]",idx_to_execute, incarnation);
                    updates_outside = true;
                }
                versioned_data_cache.add_write(&k, write_version, v);
                // if updates_outside{
                //     print!("[Updates({:?}){}-{}:{:?}]",std::thread::current().name(),idx_to_execute, incarnation,k);
                // }
            }
            ...
            //    if !prev_modified_keys.remove(&k) {
                    //print!("[~{},{}deltas:{:?}~]",idx_to_execute, incarnation,k);
                    //print!("[~{},{}deltas]",idx_to_execute, incarnation);
            //        updates_outside = true;
            //    }
        ...
        let result = match execute_result {
            ...
            ExecutionStatus::Success(output) => {
                ...
                if output.get_status().is_discarded(){
                    let e = if let Err(code) = output.get_status().status(){
                        //xuq
                        //print!("[suc-dis({:?}):{:?}]",std::thread::current().name(),code);
                        code
                    }else{
                        StatusCode::UNKNOWN_STATUS
                    };
                    
                    ExecutionStatus::Abort(Error::NONCE_TOO_NEW)
                }else{
                ...
                //{
                //    let ot:HashSet<<T as Transaction>::Key> = output.get_writes()
                //        .into_iter()
                //        .map(|(k, _)| k)
                //        .collect();
                    
                //    if ot.len() == 0{
                //        print!("[success:null:{:?}]",ot);
        ...
        if updates_outside == true{
            //xuq
            //print!("[outside_pmk({:?}){}-{}:{:?}]",std::thread::current().name(),idx_to_execute, incarnation,prev_modified_keys);
        }
        // Remove entries from previous write/delta set that were not overwritten.
        for k in prev_modified_keys {
            //print!("[delete{}-{}:{:?}]",idx_to_execute, incarnation,k);
            versioned_data_cache.delete(&k, idx_to_execute);
        }
        
        last_input_output.record(idx_to_execute, state_view.take_reads(), result);
        let ret = scheduler.finish_execution(idx_to_execute, incarnation, updates_outside, guard);
        //println!("[Record({:?}){}-{}:{:?}]",std::thread::current().name(),idx_to_execute,incarnation, last_input_output.modified_keys(idx_to_execute));
        // match last_input_output.outputs[idx_to_execute]{
        //     ExecutionStatus::Success(s) => {
        //         //print!("recode S:{:?}", s.get_writes().iter().map(|(k, _)|*k).collect::<<T as task::Transaction>::Key>());
        //         for ss in  s.get_writes(){
        //             print!("[recode S:{:?}]",ss.0);
        //         }
        //     },
        //     ExecutionStatus::Abort(a) => {
        //       print!("recodrd error!")  
        //     },
        //     ExecutionStatus::SkipRest(s) => {
        //         //print!("recode SK:{:?}", s.get_writes().iter().map(|(k, _)|k).collect());
        //         for ss in  s.get_writes(){
        //             print!("[recode K:{:?}]",ss.0);
        //         }
        //     },
        // };
        //xuq
        //println!("[Eend({:?})]",std::thread::current().name());
        ret
    }
    ...
    fn validate<'a>(
        ...
        //xuq
        //print!("[Validate({:?}):{}-{}]",std::thread::current().name(),idx_to_validate, incarnation);
        ...
        let valid = read_set.iter().all(|r| {
            match versioned_data_cache.read(r.path(), idx_to_validate) {
                Ok(Version(version, _)) => {
                    if r.validate_version(version){
                        true
                    } else{
                        counters::V_version_COUNT.inc();
                        //xuq
                        //print!("[VersionErr({:?}):{}]",std::thread::current().name(),idx_to_validate);
                        false
                    }},
                Ok(Resolved(value)) => {
                    if r.validate_resolved(value){
                        true
                    } else{
                        counters::V_Resolved_COUNT.inc();
                        //print!("Resolved");
                        false
                    }
                },
                Err(Dependency(_)) => {
                    counters::V_Dependency_COUNT.inc();
                    //print!("Dependency");
                    false
                }, // Dependency implies a validation failure.
                Err(Unresolved(delta)) => {
                    
                    if r.validate_unresolved(delta){
                        true
                    } else{
                        counters::V_Unresolved_COUNT.inc();
                        //print!("Unresolved");
                        false
                    }
                },
                Err(NotFound) => {
                    ...
                    if r.validate_storage(){
                        true
                    } else{
                        counters::V_NotFound_COUNT.inc();
                        //print!("NotFound");
                        false
                    }
                },
                ...
                Err(DeltaApplicationFailure) => {
                    ...
                    if r.validate_delta_application_failure(){
                        true
                    } else{
                        counters::V_DeltaApplicationFailure_COUNT.inc();
                        //print!("DeltaApplicationFailure");
                        false
                    }
                },
            }
        ...
        // if !valid{
        //     if let  Some(status) = last_input_output.get_error(idx_to_validate) {
        //         // if !status {
        //         //     counters::SPECULATIVE_ABORT_COUNT.inc();
        //         //     let ret = scheduler.finish_abort(idx_to_validate, incarnation, guard);
        //         //     return  ret
        //         // }
    
        //         if status == StatusCode::SEQUENCE_NUMBER_TOO_NEW && !valid {
        //             if scheduler.check_validate(idx_to_validate){
        ...
        if aborted {
            //xuq
            //print!("[Aborted({:?}):{}{:?}]",std::thread::current().name(),idx_to_validate,valid);
            counters::SPECULATIVE_ABORT_COUNT.inc();
            ...
            let ret = scheduler.finish_abort(idx_to_validate, incarnation, guard);
            //xuq
            //println!("[Vend({:?})]",std::thread::current().name());
            ret
        } else {
            //xuq
            //println!("[Vend2NoTask({:?})]",std::thread::current().name());
    ...
    fn work_task_with_scope(
       ...
       // println!("execute_transactions_parallel: {} ms",start.elapsed().as_millis());
        ...
        loop {
            scheduler_task = match scheduler_task {
                ...
                SchedulerTask::ExecutionTask(_, Some(condvar), _guard) => {
                    print!("[Dependency({:?}):]",std::thread::current().name());
    ...
    > {
        ...
        //let start = std::time::Instant::now();
       // println!("execute_transactions_parallel: {} ms",start.elapsed().as_millis());
       counters::SPECULATIVE_ABORT_COUNT.reset();
       counters::V_version_COUNT.reset();
       counters::V_Resolved_COUNT.reset();
       counters::V_Dependency_COUNT.reset();
       counters::V_Unresolved_COUNT.reset();
       counters::V_NotFound_COUNT.reset();
       counters::V_DeltaApplicationFailure_COUNT.reset();
       ...
       let start = std::time::Instant::now();
        ...
        println!("work_task_with_scope: {} ms",start.elapsed().as_millis());
        println!("{:?}",counters::V_version_COUNT.get());
        println!("{:?}",counters::V_Resolved_COUNT.get());
        println!("{:?}",counters::V_Dependency_COUNT.get());
        println!("{:?}",counters::V_Unresolved_COUNT.get());
        println!("{:?}",counters::V_NotFound_COUNT.get());
        println!("{:?}",counters::V_DeltaApplicationFailure_COUNT.get());
