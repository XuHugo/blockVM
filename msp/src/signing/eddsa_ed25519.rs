...
impl Context for EddsaEd25519Context {
    ...
    fn get_public_key(&self, private_key: &dyn PrivateKey) -> Result<Box<dyn PublicKey>, Error> {
        ...
        let result = EddsaEd25519PublicKey::from_hex(
            bytes_to_hex_str(ed25519_dalek::PublicKey::from(&sk).as_bytes()).as_str(),
        );
        match result {
            Err(err) => Err(err),
            Ok(pk) => Ok(Box::new(pk)),
    ...
    fn new_random_private_key(&self) -> Result<Box<dyn PrivateKey>, Error> {
        ...
        Ok(Box::new(EddsaEd25519PrivateKey {
